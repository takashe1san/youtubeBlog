    ...
    <form method="POST" action="{{ route('dashboard.post.update' , $post)}}" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                ...
                @enderror
                <img src="{{asset($post->image)}}" alt="category picture" width="100" srcset="">
            </div>
        </div>
                  ...
                  @foreach (config('app.languages') as $key => $lang)
                    <div class="tab-pane fade @if($loop->index == 0) show active in @endif" id="{{$key}}" role="tabpanel" aria-labelledby="home-tab">
                      ...
                      <div class="form-group col-md-12">
                        ...
                        <textarea name="{{$key}}[smallDesc]" class="form-control" id="editor" cols="30" rows="4">{{$post->translate($key)->smallDesc}}</textarea>
                      </div> 

                      <div class="form-group col-md-12">
                        <label>{{__('words.content')}}</label>
                        <textarea name="{{$key}}[content]" class="form-control" id="editor" cols="30" rows="10">{{$post->translate($key)->content}}</textarea>
                      </div> 
